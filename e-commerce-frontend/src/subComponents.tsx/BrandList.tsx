        ...
        <tr key={item.brandID}>
          <td className="border border-gray-400 p-2">
            <div className="flex items-center space-x-2">
              <img src={item.brandThumbnail} className="w-15 h-15 shadow-lg " />
              <span>{item.brandName}</span>
            </div>
          </td>
          <td className="border border-gray-400 p-2">
            <div className="flex space-x-2">
              <img src={editIcon} className="w-10 h-10 p-1 cursor-pointer" onClick={()=>{setEditBrand(item); setToggleModal(true)}}/>
              <img src={deleteIcon} className="w-10 h-10 p-1 cursor-pointer" onClick={()=>{setDeleteBrandID(item.brandID); setIsDelete(true)}}/>
            </div>
          </td>
        </tr>
      ))}
